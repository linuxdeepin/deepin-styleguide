...
\DBox {
  在 \cppinline{.cpp} 文件中定义一个不需要被外部引用的变量时，可以将它们放在匿名命名空间或声明为 \cppinline{static} 。但是不要在
  \cppinline{.h} 文件中这么做。
}
...
:ref:`namespaces` 。举例而言，对于头文件 \cppinline{myproject/foo_bar.h}  , 应当使用
...
如果你必须定义非成员函数, 又只是在 \cppinline{.cpp} 文件中使用它, 可使用匿名 :ref:`namespaces` 或
\cppinline{static} 链接关键字 (如 \cppinline{static int Foo() {...}}) 限定其作用域.
...
属于 \cppinline{if}, \cppinline{while} 和 \cppinline{for}
